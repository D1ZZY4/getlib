...
describe("detectIntent required-arg fallbacks", () => {
  it("falls back to gl_search when a migration verb has no parseable library", () => {
  ...
  it("falls back to gl_search when a compare verb lacks two library names", () => {
    const intent = detectIntent({ query: "compare frameworks" });
    expect(intent.tool).toBe("gl_search");
  });

  it("falls back to gl_search when a changelog verb has no library", () => {
  ...
  it("never recommends a tool whose required identifier is missing", () => {
    ...
    for (const query of queries) {
      ...
      if (["gl_migration", "gl_changelog", "gl_get_docs", "gl_best_practices", "gl_examples", "gl_snippets"].includes(intent.tool)) {
        expect(intent.args["libraryId"] ?? intent.args["library"]).toBeTruthy();
      }
      if (intent.tool === "gl_compare") {
