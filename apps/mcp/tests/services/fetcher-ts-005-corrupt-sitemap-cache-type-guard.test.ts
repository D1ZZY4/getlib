...
import { resetAllCircuits } from "../../src/services/circuit-breaker.js";
...
beforeEach(async () => {
  ...
  const { docCache, diskDocCache } = await import("../../src/services/cache.js");
  docCache.clear();
...
describe("TS-005: corrupt sitemap cache type guard", () => {
  it("returns [] when cached value is JSON null", async () => {
    const { docCache } = await import("../../src/services/cache.js");
...
describe("docsifyToRaw", () => {
  it("rewrites a docsify hash route to the raw markdown path", async () => {
    const { docsifyToRaw } = await import("../../src/services/fetcher.js");
