...
import { resetTelemetry, getRecentOutcomes } from "../../src/services/telemetry.js";
...
describe("gl_dispatch tool", () => {
  ...
  it("registers without error", () => {
    ...
    expect(() => getHandler(server, "gl_dispatch")).not.toThrow();
  });

  it("returns routing decision for 'use gl'", async () => {
    const server = new McpServer({ name: "test", version: "0.0.0" });
    registerDispatchTool(server);
    const handler = getHandler(server, "gl_dispatch");
    const result = (await handler({ query: "use gl" })) as {
      content: Array<{ text: string }>;
      structuredContent?: { tool: string; confidence: number };
    };
    expect(result.content[0]?.text).toContain("gl_auto_scan");
    expect(result.structuredContent?.tool).toBe("gl_auto_scan");
    expect(result.structuredContent?.confidence).toBeGreaterThan(0.5);
  });

  it("returns routing decision for 'use gl for next.js'", async () => {
    const server = new McpServer({ name: "test", version: "0.0.0" });
    registerDispatchTool(server);
    const handler = getHandler(server, "gl_dispatch");
    const result = (await handler({ query: "use gl for next.js" })) as {
      structuredContent?: { tool: string; args: Record<string, unknown> };
    };
    expect(result.structuredContent?.tool).toBe("gl_best_practices");
  ...
  it("records telemetry on every invocation", async () => {
    ...
    registerDispatchTool(server);
    const handler = getHandler(server, "gl_dispatch");
    await handler({ query: "use gl" });
    const outcomes = getRecentOutcomes();
    expect(outcomes.length).toBe(1);
    expect(outcomes[0]?.tool).toBe("gl_dispatch");
  ...
  it("includes routing table in response text", async () => {
    ...
    registerDispatchTool(server);
    const handler = getHandler(server, "gl_dispatch");
    const result = (await handler({ query: "use gl" })) as { content: Array<{ text: string }> };
  ...
  it("respects projectPath argument for project-level routing", async () => {
    ...
    registerDispatchTool(server);
    const handler = getHandler(server, "gl_dispatch");
    const result = (await handler({ query: "use gl", projectPath: "/tmp" })) as {
