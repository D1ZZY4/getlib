...
export const NOISE_PHRASES = [
  // "use" is noise ("use gl for react") EXCEPT in "can i use X" — that is the
  ...
  /\b(?:gl(?:\s*[-_]?\s*mcp)?|getlib(?:\s*mcp)?|the\s+gl|the\s+gl[-_]?mcp)\b/gi,
  /\bplease\b/gi,
];

/** Single-word tokens that strongly signal a tool, sorted by precision */
export const VERB_HINTS: Array<{ tool: GlToolName; words: string[] }> = [
  { tool: "gl_audit", words: ["audit", "find issues", "find bugs", "check issues", "find problems", "code issues", "review code", "scan code", "scan source", "find all the issues", "find all issues", "find all bugs", "any issues", "any bugs"] },
  { tool: "gl_migration", words: ["migrate", "migration", "upgrade", "upgrading", "breaking change", "breaking changes", "move from", "switch to"] },
  { tool: "gl_changelog", words: ["changelog", "release notes", "what's new", "whats new", "new in", "recent changes"] },
  { tool: "gl_compare", words: ["compare", "vs", "versus", "differences between", "which one", "or better"] },
  { tool: "gl_compat", words: ["browser support", "browser compatibility", "compatibility", "supported in", "works on", "caniuse", "can i use", "does safari", "does chrome", "does firefox", "does edge", "does node", "safari support", "chrome support", "firefox support", "edge support", "node support", "which browsers", "baseline status"] },
  { tool: "gl_examples", words: ["example", "examples", "real-world", "real world", "show me code", "sample"] },
  { tool: "gl_snippets", words: ["snippets", "snippet", "code snippets", "snippet index"] },
  { tool: "gl_best_practices", words: ["best practices", "best-practices", "patterns", "recommendations", "guidelines", "tips for"] },
  { tool: "gl_auto_scan", words: ["scan project", "scan dependencies", "scan deps", "all my deps", "all dependencies", "every dependency", "every library"] },
  { tool: "gl_batch_resolve", words: ["resolve multiple", "lookup all", "batch lookup", "resolve every"] },
  { tool: "gl_search", words: ["search for", "find docs about", "look up topic", "what is", "explain"] },
  { tool: "gl_get_docs", words: ["docs for", "documentation for", "official docs", "fetch docs", "read docs"] },
  { tool: "gl_resolve_library", words: ["resolve", "lookup", "look up", "find library", "where is"] },
];
