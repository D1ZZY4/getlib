...
export function handleCliFlags(args: string[]): void {
    ...
    if (args.includes("--help") || args.includes("-h")) {
      process.stdout.write([
        ...
        "",
        "Environment:",
        "  GL_HTTP_PORT       Enable HTTP transport on port",
        "  GL_HTTP_STATEFUL   Set =1 for session-per-request mode",
        "  GL_AUTH_TOKEN      Bearer token required for HTTP endpoints",
        "  GL_GITHUB_TOKEN    GitHub API token for higher rate limits",
        "  GL_CACHE_DIR       Disk cache directory (default ~/.gl-mcp-cache)",
        "",
