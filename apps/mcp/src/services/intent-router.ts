/**
 * Intent router — maps a plain-text user query like "use gl mcp" or
 ...
 *    when they want gl-mcp to figure out the right action.
...
import type { GlToolName, IntentInput, IntentMatch } from "./intent/types.js";
...
export type { GlToolName, IntentInput, IntentMatch } from "./intent/types.js";
...
export function detectIntent({ query, projectPath }: IntentInput): IntentMatch {
  ...
  // 5. Empty / "use gl" / project-level → auto-scan
  if (isProjectLevelInvocation(raw)) {
    return {
      tool: "gl_auto_scan",
      args: { projectPath: projectPath ?? "." },
      reason: 'project-level invocation ("use gl", "scan this project", etc.)',
      confidence: 0.8,
    };
  }
