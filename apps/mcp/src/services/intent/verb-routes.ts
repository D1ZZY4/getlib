...
import type { GlToolName, IntentMatch } from "./types.js";
...
export function routeVerb(
  top: { tool: GlToolName; word: string },
  ctx: VerbContext,
): IntentMatch {
...
switch (top.tool) {
  ...
  default: {
    ...
    if (top.tool === "gl_snippets" && library) args["libraryId"] = library.id;
    return {
      tool: top.tool,
      args,
